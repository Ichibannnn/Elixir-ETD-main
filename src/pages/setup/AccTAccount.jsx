...
const Department = () => {
  ...
  const fetchAccountApi = async (pageNumber, pageSize, status, search) => {
    const response = await request.get(`Account/GetAllAccountWithPaginationOrig/${status}?PageNumber=${pageNumber}&PageSize=${pageSize}&search=${search}`);
        ...
        <Flex flexDirection="column" gap={1} w="full">
          ...
          <Flex w="full" flexDirection="column" gap={2}>
                  ...
                  <Tbody>
                    {account.account?.map((acc, i) => (
                      <Tr key={i}>
                        ...
                        <Td pl={0}>
                          <HStack>
                            <Button bg="none" onClick={() => editAccountHandler(acc)}>
                              <AiTwotoneEdit />
                            </Button>
                            <Popover>
                              {({ onClose }) => (
                                    ...
                                    <PopoverContent bg="primary" color="#fff">
                                      ...
                                      <PopoverBody>
                                        <VStack onClick={onClose}>
                                          {acc.isActive === true ? (
                                            <Text>Are you sure you want to set this account inactive?</Text>
                                          ) : (
                                            <Text>Are you sure you want to set this account active?</Text>
                                          )}
                                          <Button colorScheme="green" size="sm" onClick={() => changeStatusHandler(acc.id, acc.isActive)}>
                                            Yes
                                          </Button>
                                        </VStack>
                                      </PopoverBody>
            ...
            <Flex justifyContent="space-between">
              ...
              {isOpen && <DrawerComponent isOpen={isOpen} onClose={onClose} getAccountHandler={getAccountHandler} editData={editData} disableEdit={disableEdit} />}
    ...
    try {
      if (data.formData.id === "") {
        ...
        const res = await request
          ...
          .then((res) => {
            ToastComponent("Success", "New account created!", "success", toast);
            getAccountHandler();
            onClose();
          })
          .catch((err) => {
            ...
            <DrawerBody>
              <Stack spacing="7px">
                ...
                <Box>
                  ...
                  <Input {...register("formData.accountName")} placeholder="Please enter Account Name" autoComplete="off" />
                  <Text color="red" fontSize="xs">
