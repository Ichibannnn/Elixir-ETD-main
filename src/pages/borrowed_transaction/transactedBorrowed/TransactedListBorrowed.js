...
const fetchTransactBorrowedApi = async (pageNumber, pageSize, search, status) => {
  const res = await request.get(`Borrowed/GetAllReturnedItemOrig?pageNumber=${pageNumber}&pageSize=${pageSize}&search=${search}&status=${status}`);
  return res.data;
};
...
export const TransactedListBorrowed = () => {
  ...
  const fetchTransactedBorrowed = () => {
    fetchTransactBorrowedApi(currentPage, pageSize, search, status).then((res) => {
  ...
  return (
    <Flex justifyContent="center" flexDirection="column" w="full">
            ...
            <Tbody>
              {transactedBorrowData?.issue?.map((borrow, i) => (
                <Tr key={i}>
                  ...
                  <Td fontSize="xs">
                    <Button onClick={() => viewHandler(borrow.id)} colorScheme="blue" size="xs">
                      View
                    </Button>
                  </Td>
                </Tr>
      ...
      {isView && <ViewModal isOpen={isView} onClose={closeView} statusBody={statusBody} isLoading={isLoading} setIsLoading={setIsLoading} />}
    </Flex>
  );
};
