...
import { ApproveModal, RejectModalApproval, ViewModalApproval } from "./ActionModal";
...
const fetchBorrowedApprovalApi = async (pageNumber, pageSize, search, status) => {
  const res = await request.get(`Borrowed/GetAllForApprovalBorrowedWithPaginationOrig?pageNumber=${pageNumber}&pageSize=${pageSize}&search=${search}&status=${status}`);
  return res.data;
};
                    ...
                    <Flex justifyContent="center">
                      <Box>
                        <Menu>
                          ...
                          <MenuList>
                            <MenuItem icon={<GrView fontSize="17px" />} onClick={() => viewHandler(borrow.borrowedPKey)}>
                              <Text fontSize="15px">View</Text>
                            </MenuItem>
                            <MenuItem icon={<BsCheck2Square fontSize="17px" color="green" />} onClick={() => approveHandler(borrow.borrowedPKey)}>
                              <Text fontSize="15px" color="green">
                                Approve
                              </Text>
                            </MenuItem>
                            <MenuItem icon={<GiCancel fontSize="17px" color="red" />} onClick={() => rejectHandler(borrow.borrowedPKey)}>
