...
import { ApproveModal, RejectModal, ViewModal } from "./ActionButtonModal";

const currentUser = decodeUser();
const userId = currentUser?.id;

const fetchFuelApprovalApi = async (pageNumber, pageSize, search) => {
...
const ApproverPendingTab = () => {
  ...
  const fetchFuelApproval = () => {
    fetchFuelApprovalApi(currentPage, pageSize, search).then((res) => {
      setFuelData(res);
  ...
  return (
    <Flex justifyContent="center" flexDirection="column" mb="150px" w="full" p={5}>
                      ...
                      <Box>
                        <Menu>
                          ...
                          <MenuList>
                            ...
                            <MenuItem icon={<MdCheck fontSize="17px" color="green" />} onClick={() => approveHandler(item)}>
                              <Text fontSize="15px" color="green" _hover={{ color: "green" }}>
                                Approved
                              </Text>
                            </MenuItem>
                            <MenuItem icon={<GiCancel fontSize="17px" color="red" />} onClick={() => rejectHandler(item)}>
      ...
      {isApprove && <ApproveModal isOpen={isApprove} onClose={closeApprove} data={getData} fetchFuelApproval={fetchFuelApproval} />}
      {isReject && <RejectModal isOpen={isReject} onClose={closeReject} data={getData} fetchFuelApproval={fetchFuelApproval} isLoading={isLoading} setIsLoading={setIsLoading} />}
    </Flex>
  );
};

export default ApproverPendingTab;
