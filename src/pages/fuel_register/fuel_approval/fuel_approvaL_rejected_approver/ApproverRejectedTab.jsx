...
const fetchFuelRejectedApi = async (pageNumber, pageSize, search) => {
  const res = await request.get(`FuelRegister/page?PageNumber=${pageNumber}&PageSize=${pageSize}&Search=${search}&Status=Rejected`);
  return res.data;
};

const ApproverRejectedTab = () => {
  ...
  const fetchFuelRejected = () => {
    fetchFuelRejectedApi(currentPage, pageSize, search).then((res) => {
      setFuelData(res);
  ...
  return (
    <Flex justifyContent="center" flexDirection="column" mb="150px" w="full" p={5}>
      ...
      <Flex flexDirection="column" mt={2}>
        <Box w="full" bgColor="primary" h="22px">
          <Text fontWeight="normal" fontSize="13px" color="white" textAlign="center" justifyContent="center">
            List of Rejected Fuel Requests
          </Text>
        </Box>
      ...
      {/* {isCancel && (
        <CancelRequestModal isOpen={isCancel} onClose={closeCancel} data={getData} fetchFuelRejected={fetchFuelRejected} isLoading={isLoading} setIsLoading={setIsLoading} />
      )}

      {isCreate && <AddModal isOpen={isCreate} onClose={closeCreate} data={getData} fetchFuelRejected={fetchFuelRejected} />} */}
    </Flex>
  );
};

export default ApproverRejectedTab;
