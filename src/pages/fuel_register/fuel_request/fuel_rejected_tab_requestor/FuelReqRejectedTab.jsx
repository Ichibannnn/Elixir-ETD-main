...
const fetchFuelReqApprovedApi = async (pageNumber, pageSize, search) => {
  const res = await request.get(`FuelRegister/page?PageNumber=${pageNumber}&PageSize=${pageSize}&Search=${search}&Status=Rejected&UserId=${userId}`);
  return res.data;
};

const FuelReqRejectedTab = () => {
  ...
  const fetchFuelReqApproved = () => {
    fetchFuelReqApprovedApi(currentPage, pageSize, search).then((res) => {
      setFuelData(res);
...
export default FuelReqRejectedTab;
